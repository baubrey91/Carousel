...
public class CarouselCollectionView: UICollectionView {
    ...
    private func getRealIndex(_ fakeIndexPath: IndexPath) -> Int {
        guard let realNumberOfItems = carouselDataSource?.numberOfItems,
            fakeIndexPath.row != realNumberOfItems + 1 else {
            return 0
        }

        return (fakeIndexPath.row == 0 ? realNumberOfItems : fakeIndexPath.row) - 1
    }
